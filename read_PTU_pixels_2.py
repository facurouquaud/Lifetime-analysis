...
#%%
archivo = "C:\\Users\\Luis1\\Downloads\\Fotobeadsarribaizq.ptu"
# archivo = "C:\\Users\\Lenovo\\Downloads\\pruebaDAQ100p.ptu"
with open(archivo, 'rb') as fd:
    numRecords, _, _ = readHeaders(fd)
    dtime, truesync, pixeles = readPT3_fast_pixels(fd, numRecords)
pixeles_totales,pixeles_ida,pixeles_vuelta = filtrar_pixels(pixeles[36:len(pixeles) - 36],500,4,1)
...
c = 10000
cuentas = np.array([len(x) for x in pixeles_vuelta])  

# recortar o rellenar como antes
if len(cuentas) < n_pix_objetivo:
    cuentas = np.pad(cuentas, (0, n_pix_objetivo - len(cuentas)), mode='constant', constant_values=0)
elif len(cuentas) > n_pix_objetivo:
    cuentas = cuentas[c :n_pix_objetivo + c]

imagen = cuentas.reshape(shape)

# ---> CORRECCIÓN CLAVE: construir ejes con la correspondencia correcta
# columnas (x) tienen length = shape[1]; filas (y) tienen length = shape[0]
